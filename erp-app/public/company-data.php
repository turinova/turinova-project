<?php
/**
 * Company Data Entry Point
 */

// Load configuration first
require_once '../config/app.php';
require_once '../config/session.php';

// Start session after configuration
session_start();

// Load database connection
require_once '../database/connection.php';

// Load helpers
require_once '../app/helpers/validation.php';
require_once '../app/helpers/flash.php';
require_once '../app/helpers/auth.php';

// Require authentication and page access
requirePageAccess('company_data');

// Check if this is an update action
if (isset($_GET['action']) && $_GET['action'] === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    // Load the controller and call update method
    require_once '../app/controllers/CompanyDataController.php';
    $controller = new CompanyDataController();
    $controller->update();
    exit;
}

// Set path for routing
$path = '/company-data';

// Load routes
require_once '../routes/web.php';
?> 